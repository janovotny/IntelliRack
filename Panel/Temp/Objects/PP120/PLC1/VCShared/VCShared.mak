...
LIB_SHARED=$(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_PLC)/VCShared/vcshared.vca

#
# Shared Runtime Options
#
VCRS_OBJECT=$(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_PLC)/VCShared/vcrt_s.vco
...
# Datapoint Objects
#
$(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_PLC)/VCShared/dso.Internal.vco: $(AS_PROJECT_PATH)/Logical/VCShared/DataSources/Internal.dso 
	 $(VCC) -f '$<' -o '$@' $(DSOFLAGS) $(VCCFLAGS_Visu) -p Visu -vcr 3950

$(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_PLC)/VCShared/dso.DataSource.vco: $(AS_PROJECT_PATH)/Logical/VCShared/DataSources/DataSource.dso 
	 $(VCC) -f '$<' -o '$@' $(DSOFLAGS) $(VCCFLAGS_Visu) -p Visu -vcr 3950

DPT_OBJECTS = $(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_PLC)/VCShared/dpt.DataPointList.vco
DSO_OBJECTS_Visu=$(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_PLC)/VCShared/dso.Internal.vco $(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_PLC)/VCShared/dso.DataSource.vco 
...
$(VCRS_OBJECT) : $(VCRS_SOURCE)
	$(VCC) -f '$<' -o '$@' -ct shared -cv '$(AS_PROJECT_PATH)/Logical/VCShared/ControlVersion.cvinfo' -sl  $(VCCFLAGS_Visu) -p Visu -vcr 3950

#
# The Shared Module
#
SHARED_MODULE=$(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_PLC)/vcshared.br
SHARED_CCF=$(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_PLC)/VCShared/vcshared.ccf
DEL_SHARED_CCF=$(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_PLC)/VCShared/vcshared.ccf.lfl

$(SHARED_MODULE) : $(SHARED_CCF)
	 $(MODGEN) -m $(VCSTARTUP) -v V1.00.0 -vc '$(VCOBJECT_Visu)' -f '$<' -o '$@' -d vcgclass -profile 'False'
...
$(SHARED_CCF): $(VCRS_OBJECT) $(VCR_OBJECTS_Visu) $(VCUG_OBJECTS_Visu) $(ALGRP_OBJECTS_Visu) $(ALCFG_OBJECTS_Visu) $(DSO_OBJECTS_Visu) $(TXTGRP_SHARED_OBJECTS_Visu) $(CVINFO_OBJECTS_Visu)
	-@CMD if exist /Q "$(DEL_SHARED_CCF)" /C DEL /F /Q "$(DEL_SHARED_CCF)" 2>nul
	 ...
	 @$(VCFLGEN) '$@.lfl' '$(VCUG_OBJECTS_Visu:.vco=.vco,)' -temp '$(TEMP_PATH_Visu)' -prj '$(PRJ_PATH_Visu)'
	 @$(VCFLGEN) '$@.lfl' '$(ALGRP_OBJECTS_Visu:.vco=.vco,)' -temp '$(TEMP_PATH_Visu)' -prj '$(PRJ_PATH_Visu)'
	 @$(VCFLGEN) '$@.lfl' '$(ALCFG_OBJECTS_Visu:.vco=.vco,)' -temp '$(TEMP_PATH_Visu)' -prj '$(PRJ_PATH_Visu)'
	 @$(VCFLGEN) '$@.lfl' '$(TXTGRP_SHARED_OBJECTS_Visu:.vco=.vco,)' -temp '$(TEMP_PATH_Visu)' -prj '$(PRJ_PATH_Visu)'
	 ...
	 $(LINK) '$@.lfl' -o '$@' -lib '$(LIB_SHARED)' -P '$(AS_PROJECT_PATH)' -m 'shared resources' -profile 'False' -warningLevel2 -name Visu -sfas
